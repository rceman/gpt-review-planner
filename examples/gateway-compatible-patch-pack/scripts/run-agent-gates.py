...
def main():
    ...
    if subprocess.check_output(["git","-C",str(repo),"status","--porcelain"],text=True): fail("worktree must be clean")
    outdir=Path(a.output_dir); outdir.mkdir(parents=True,exist_ok=True); started=datetime.now(timezone.utc).isoformat(); results=[]; overall="pass"
    for gate in plan["gates"]:
        ...
        for step in gate["steps"]:
            ...
            try:
                ...
                metrics=count_output(step["parser"],stdout.decode(errors="replace")+stderr.decode(errors="replace"))
                gr["metrics"].update(metrics)
                if code: status="fail"
    ...
    result={"schema_version":1,"implementation_commit":a.implementation_commit,"started_at":started,"completed_at":datetime.now(timezone.utc).isoformat(),"status":overall,"gates":results}
