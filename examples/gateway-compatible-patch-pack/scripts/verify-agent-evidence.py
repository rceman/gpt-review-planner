...
def read_committed_evidence(repo: Path, commit: str, directory: Path, pack_manifest_raw: bytes) -> dict[str, Any]:
    ...
    try:
        evidence = json.loads(evidence_raw.decode("utf-8"), object_pairs_hook=strict_object)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
...
def worktree_status(repo: Path) -> dict[str, str]:
    ...
    while index < len(fields):
        ...
        status = text[:2]
        path = normalize_repo_path(text[3:], "git status path")
        if any(code in status for code in ("R", "C")):
            raise EvidenceError("evidence preparation must not contain rename or copy statuses")
        result[path] = status
    return result


def validate_prepared_paths(repo: Path, directory: Path, expected: set[str]) -> None:
    status = worktree_status(repo)
    if set(status) != expected:
        missing = sorted(expected - set(status))
        extra = sorted(set(status) - expected)
        raise EvidenceError(f"prepared evidence path mismatch: missing={missing!r}, extra={extra!r}")
    invalid = {path: value for path, value in status.items() if value not in {"??", "A "}}
    if invalid:
        raise EvidenceError(f"prepared evidence files must be untracked or staged additions: {invalid!r}")
    directory_path = repo / directory
    actual_files = {path.name for path in directory_path.iterdir() if path.is_file() and not path.is_symlink()}
    if actual_files != set(EVIDENCE_FILES):
        raise EvidenceError(f"evidence directory must contain exactly {sorted(EVIDENCE_FILES)!r}")
    if any(path.is_symlink() or not path.is_file() for path in (directory_path / name for name in EVIDENCE_FILES)):
        raise EvidenceError("evidence files must be regular non-symlink files")
    staged_expected = {path for path, value in status.items() if value == "A "}
    staged_actual = diff_scope(repo, "", "", cached=True)
    assert_scope("staged evidence", (staged_expected, set(), set()), staged_actual)
...
def command_prepare(pack: Path, repo: Path, implementation: str) -> None:
    ...
    validate_prepared_paths(repo, directory, expected_paths)
    print(f"PASS: prepared JSON evidence for {implementation}")
