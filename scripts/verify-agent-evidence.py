...
def validate_proof(repo: Path,base: str,implementation: str,proof: dict) -> None:
    ...
    if kind in {"source","test","workflow","documentation"}:
        ...
        if hashlib.sha256(selected).hexdigest()!=expected:
            raise EvidenceError(f"proof hash mismatch: {path}")
...
def validate(repo: Path,manifest: dict,evidence: dict,implementation: str) -> None:
    ...
    if [item.get("id") for item in evidence_requirements]!=manifest_requirements:
        raise EvidenceError("requirement IDs mismatch")
    if evidence.get("deviations")!=[]:
        raise EvidenceError("accepted canonical evidence must have no deviations")
    ...
    for gate in evidence["gates"]:
        if gate.get("status")!="pass" or gate.get("exit")!=0:
            raise EvidenceError(f"gate failed: {gate.get('id')}")
...
def main() -> int:
    ...
    manifest,manifest_raw=load(pack/"MANIFEST.json")
