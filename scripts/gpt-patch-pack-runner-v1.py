...
CANONICAL_PREFLIGHT = [
    "python3", "{pack}/payload/apply.py", "--check",
    "--patch", "{pack}/payload/changes.patch",
]
CANONICAL_APPLY = [
    "python3", "{pack}/payload/apply.py", "--apply",
    "--patch", "{pack}/payload/changes.patch",
]
...
def run(
    ...
    *,
    ...
    try:
        ...
        raise PackError(f"command timed out after {timeout}s: {argv!r}") from exc
    stdout = proc.stdout or ""
    stderr = proc.stderr or ""
    if len(stdout.encode()) + len(stderr.encode()) > output_limit:
...
def safe_extract(archive: Path, destination: Path) -> Path:
    ...
    with tarfile.open(archive, "r:gz") as tf:
        ...
        for member in members:
            ...
            if member.isfile() and stat.S_IMODE(member.mode) not in {0o600, 0o644, 0o700, 0o755}:
...
def validate_manifest(pack: Path) -> dict[str, Any]:
    ...
    if not isinstance(value["patch_id"], str) or not value["patch_id"].startswith("patch-"):
    ...
    exact_keys(payload, {"patch", "preflight", "apply", "success_marker"}, "payload")
    if payload["patch"] != "payload/changes.patch":
        raise PackError("payload patch path is not canonical")
    preflight = validate_command(payload["preflight"], "payload.preflight")
    apply = validate_command(payload["apply"], "payload.apply")
    if preflight["argv"] != CANONICAL_PREFLIGHT or apply["argv"] != CANONICAL_APPLY:
        raise PackError("payload command is not canonical")
    if payload["success_marker"] != "GPT_PATCH_PAYLOAD_APPLIED":
        raise PackError("payload success marker invalid")
    ...
    if not isinstance(compatibility, dict):
        raise PackError("compatibility must be an object")
    exact_keys(
        compatibility,
        {
            "scope", "authorized", "canonical_implementation",
            "legacy_behavior", "authorization_source",
            "supported_legacy_versions", "direction", "removal_condition",
        },
        "compatibility",
    )
    if compatibility["authorized"] is False:
        expected = {
            "scope": "none",
        ...
        for key, expected_value in expected.items():
            if compatibility[key] != expected_value:
                raise PackError(f"unauthorized compatibility declaration invalid: {key}")
    elif compatibility["authorized"] is True:
        if not compatibility["authorization_source"] or not compatibility["supported_legacy_versions"] or compatibility["direction"] == "none" or not compatibility["removal_condition"]:
            raise PackError("authorized compatibility declaration incomplete")
    else:
        raise PackError("compatibility.authorized must be boolean")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="gpt-patch-pack-v1-") as raw:
        ...
        try:
            ...
            output = execute(manifest["payload"]["preflight"], first, pack)
            if output:
                print(output, end="" if output.endswith("\n") else "\n")
            if git(first, "status", "--porcelain=v1", "--untracked-files=all"):
                raise PackError("preflight changed worktree A")
            output = execute(
                manifest["payload"]["apply"], first, pack,
                marker=manifest["payload"]["success_marker"],
            )
            print(output, end="" if output.endswith("\n") else "\n")
