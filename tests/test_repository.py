...
class RepositoryTest(unittest.TestCase):
    def test_required_files_exist(self) -> None:
        for relative in (
            ...
            "schemas/release-publication.schema.json",
    ...
    def test_project_archive_workflow_contract(self) -> None:
        ...
        review_only = (ROOT / required[2]).read_text(encoding="utf-8")
        ...
        self.assertIn("Already integrated project", guide)
        self.assertIn("current explicit owner instructions", guide)
        self.assertIn("full review", guide)
        self.assertIn("Review-only mode", guide)
        self.assertIn("generated by `setup.sh` or updated by `update.sh`", guide)
        self.assertIn("<IMMUTABLE_ARCHIVE_REVIEW_PROMPT_URL>", guide)
        self.assertIn("review and implement", primary.lower())
        self.assertIn("explicitly approves", primary)

        for text in (primary, review_only, preparation):
            self.assertNotRegex(text, r"\bv\d+\.\d+\.\d+\b")
            self.assertNotRegex(text, r"\b[0-9a-f]{40}\b")
            self.assertNotRegex(text, r"\b\d{8,}\b")
            self.assertNotRegex(text, r"/home/|[A-Za-z]:[\\/]")
        for phrase in (
            "<owner_task_objective>",
            "`.gpt-workflow.lock`",
            "immutable",
            "archive",
            "explicitly approves",
        ):
            self.assertIn(phrase, primary.lower())
        for phrase in (
            "Do not modify project files",
            "Do not create an Executable Patch Pack",
            "There is no Phase 2",
        ):
            self.assertIn(phrase, review_only)
        for phrase in (
            "staging",
            "owner-selected immutable",
            "official `setup.sh`",
            "never hand-write",
            "dirty worktree",
            "SHA-256",
        ...
        self.assertIn("immutable tag or commit URLs", readme)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            nested_command = command + ["--agents-file", "docs/AGENTS.md"]
            subprocess.run(nested_command, check=True, capture_output=True, text=True)
...
""",
    ...
    def test_new_patch_pack_includes_scope_verifier_and_json_evidence(self) -> None:
        manifest = json.loads((ROOT / "templates/gpt-patch-pack-v2/MANIFEST.example.json").read_text(encoding="utf-8"))
        self.assertEqual(manifest["format"], "gpt-patch-pack-v2")
        self.assertEqual(manifest["execution_mode"], "gpt_tunnel_managed")
        self.assertEqual(manifest["payload"], {"patch": "payload/changes.patch", "format": "git-binary-full-index"})
        self.assertEqual([gate["id"] for gate in manifest["gates"]], ["G1"])
    ...
    def test_template_manifest_is_valid_json(self) -> None:
        path = ROOT / "templates/gpt-patch-pack-v2/MANIFEST.example.json"
    ...
    def test_validation_report_and_manifest_separate_gpt_and_agent_evidence(self) -> None:
        report = (ROOT / "templates/gpt-patch-pack-v2/README.md").read_text(
            encoding="utf-8"
        )
        self.assertIn("build-gpt-patch-pack-v2.py", report)

        manifest = json.loads(
            (ROOT / "templates/gpt-patch-pack-v2/MANIFEST.example.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        self.assertEqual(manifest["format"], "gpt-patch-pack-v2")
        self.assertFalse((ROOT / "templates/gpt-patch-pack-v2/payload/apply.py").exists())
