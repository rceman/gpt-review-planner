...
class HostPrerequisiteContractTests(unittest.TestCase):
    ...
    def test_evidence_and_prompts_require_preflight(self):
        ...
        self.assertIn("pytest was unavailable", evidence)
        self.assertIn("status and summary contradict", evidence)
        self.assertIn("Unit, pytest, oracle", evidence)
        pack = self.read("prompts/GPT_CREATE_PATCH_PACK.md")
        self.assertIn("tool-availability preflight", pack)
        self.assertIn("required tool", pack)
