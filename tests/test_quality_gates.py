...
class QualityGatesTests(unittest.TestCase):
    ...
    def test_shell_evaluation_forms_are_rejected_but_direct_bash_check_is_valid(self):
        cases = (
            ...
            ["node", "--", "-"],
            ["node", "--test", "--", "-"],
            ["node", "--test", "--eval", "console.log('unsafe')"],
            ["node", "--help", "--unknown"],
            ["fish", "-c", "echo unsafe"],
            ["fish", "-C", "echo unsafe"],
            ["env", "MODE=x", "env", "bash", "-c", "echo unsafe"],
            ["env", "-i", "env", "bash", "scripts/check.sh"],
            ["env", "A=1", "env", "-u", "B", "bash", "scripts/check.sh"],
        )
        for argv in cases:
            value = self.valid()
        ...
        for argv in (
            ...
            ["env", "MODE=x", "bash", "scripts/check.sh"],
            ["env", "MODE=x", "python3", "-m", "compileall"],
            ["env", "MODE=x", "node", "--test"],
        ):
            value = self.valid()
            value["rules"][0]["prepare"][0]["argv"] = argv
            VALIDATOR.validate(value)
